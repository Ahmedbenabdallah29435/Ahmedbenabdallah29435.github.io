// Vanilla JS port of the react-bits "LiquidEther" WebGL fluid background.
// Original: https://reactbits.dev (React + three.js). React wrappers removed;
// call createLiquidEther(container, options) and keep the returned handle to
// pause/start/dispose the simulation.
...
`;
  ...
  class WebGLManager {
    constructor(props) {
      ...
      this.output = new Output(props.simOptions);
      this.rafId = null;
      this.running = false;
  ...
  // Pause rendering while the container is off-screen or the tab is hidden
  ...
  const ro = new ResizeObserver(() => {
    ...
    resizeRaf = requestAnimationFrame(() => webgl.resize());
  });
